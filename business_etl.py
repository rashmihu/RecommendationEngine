...
def main(input):
    ...
    business = business.filter(business["city"] != "").dropna() #dropping N/A values
    indexer = StringIndexer(inputCol="business_id", outputCol="business_id_index")
    indexed = indexer.fit(business).transform(business)
    indexed.createOrReplaceTempView("business")

    b_etl = spark.sql("SELECT business_id, business_id_index, name, city, state, latitude, longitude, stars, review_count, is_open, categories, attributes FROM business").cache()
    ...
    joined.write.parquet("yelp_etl/business_etl", mode = "overwrite")


if __name__ == '__main__':
    data_path = os.getcwd()+"/yelp_dataset/"
    Business_filepath = data_path + 'business.json'
    sc = sc(appName="Yelp")
